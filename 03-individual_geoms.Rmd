...
* The geoms discussed in this chapter a two dimensional (e.g., `x` and `y`).
* All geoms understand `color` or `colour` and size aesthetics.
* Bar, tile, and polygon understand `fill`.
* The terms above are all parameters in ggplot2 functions.
...
* What's up with `stat = "identity"`?
  ...
  - Setting this leaves the data unchanged.
...
ggplot(data_diamond_count, aes(cut, y = count)) +
  geom_bar(stat = "identity")
```

## Line chart: `geom_line()`

* Use a geom that connects points from left to right.
  - `linetype` is a useful parameter.
  - Checkout the different linetypes by 
...
* Useful when making maps: [Chapter 6](https://ggplot2-book.org/maps).
