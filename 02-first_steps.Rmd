...
```{r scatterplot-with-mpg}
ggplot(mpg, aes(cty, hwy)) + 
  geom_point()
```

```{r scatterplot-with-diamonds}
ggplot(diamonds, aes(carat, price)) + 
  geom_point()
```

```{r scatterplot-with-economics}
ggplot(economics, aes(date, unemploy)) + 
  geom_line()
```

```{r histogram-with-mpg, message=FALSE}
ggplot(mpg, aes(cty)) + 
  geom_histogram()
```
...
```{r geom-smooth-example, message=FALSE}
ggplot(mpg, aes(displ, hwy)) +
  geom_point() +
  geom_smooth()
```
...
```{r geom-histogram-and-geom-freqpoly-example}
ggplot(mpg, aes(hwy)) +
  geom_histogram()
...
```{r geom-line-and-geom-path-examples}
