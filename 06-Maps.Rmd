...
```{r, download raster, include = TRUE, eval = FALSE}
library(bomrang)
...
files <- bomrang::get_available_imagery() %>%
  stringr::str_subset("202111062300") 
...
```
NOTE: Be very specific of the file names you have downloaded from the previous step.
```{r, Set File Path, include = TRUE}
img_vis <- file.path("raster", "IDE00422.202111062300.tif")
img_inf <- file.path("raster", "IDE00421.202111062300.tif")
```

This code chuck runs the `{stars}` package. A service to ingest spacial information.
```{r, Ingest Raster, include = TRUE}
...
In the code above, the first argument specifies the path to the raster file, and the `RasterIO` argument is used to pass a list of low-level parameters to GDAL. In this case, `nBufXSize` and `nBufYSize` ensure that R reads the data at low resolution. To see what information R has imported, we can inspect the `sat_vis` object:

```{r, print sat_vis dataframe, include = TRUE}
sat_vis
```

To plot the `sat_vis` object using `ggplot2` we will apply the `geom_stars` object:

```{r, plot geom_stars, include = TRUE}
library(ggplot2)
ggplot() + 
...
```{r, plot with overlay, include = TRUE}
ggplot() + 
  geom_stars(data = sat_vis, show.legend = FALSE) +
...
```{r, transform cities df, include = TRUE}
cities <- oz_capitals %>% 
...
```{r, transform cities df 2, include = TRUE}
...
You can add the city name labels using the `geom_sf_text()` function. Though some care would be required to ensure the text is positioned nicely.

```{r, adding city text, include = TRUE}
ggplot() + 
  geom_stars(data = sat_vis, show.legend = FALSE) +
  geom_sf(data = oz_states, fill = NA, color = "white") + 
  geom_sf(data = cities, color = "red") + 
  ...
  theme_void() + 
  scale_fill_gradient(low = "black", high = "white")+
  geom_sf_text(data = cities, mapping = aes(label = city), color = "yellow") 
```
